...
function LoteDetails(props) {
    ...
    const handleOpenAddProducts = () => {
        if(openAddMoreProducts){
            setAddMoreProducts(false);
        }else{
            setAddMoreProducts(true);
        }
    }
    useEffect(() => {
        datosProductos();
    ...
    return (
        ...
        <div className='fixed inset-0 z-[50] flex flex justify-center items-center'>
             {(openAddMoreProducts) ? <AddMoreProducts  handleClick={handleOpenAddProducts} handleClose={handleOpenAddProducts} id={loteId}/> : null}
            <div className='flex flex-col bg-[#f8efe6] h-[80%] w-[80%] p-2 border-2 border-black rounded-l'>
                ...
                <div className="h-[25%] bg-[#fc7494] rounded-[10px] border-[2px] border-[#000] p-[15px] grid grid-cols-2 gap-4 mx-[20px] ">
                    ...
                    onClick={() => {handleOpenAddProducts(); setLoteId(props.id)}}>Agregar Varios Productos</button>
